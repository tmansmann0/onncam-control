...
<dict>
    ...
    <key>NSCameraUsageDescription</key><string>OnnCam Control lists and applies webcam capture formats.</string>
...
install_binary "$BINARY_NAME" "$APP/Contents/MacOS/$APP_NAME"

# Bundle app resources (if any).
APP_RESOURCES_DIR="$ROOT/Sources/$BINARY_NAME/Resources"
if [[ -d "$APP_RESOURCES_DIR" ]]; then
  cp -R "$APP_RESOURCES_DIR/." "$APP/Contents/Resources/"
fi

HELPER_ROOT="$ROOT/../onncam"
if [[ -f "$HELPER_ROOT/onncam.c" && -f "$HELPER_ROOT/Makefile" ]]; then
  make -C "$HELPER_ROOT"
  cp "$HELPER_ROOT/onncam" "$APP/Contents/Resources/onncam"
  chmod +x "$APP/Contents/Resources/onncam"
fi
